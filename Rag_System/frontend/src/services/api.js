...
api.interceptors.request.use(
  (config) => {
    if (import.meta.env.DEV) console.log('API Request:', config.method?.toUpperCase(), config.url, config.data)
    return config
  },
  (error) => {
...
api.interceptors.response.use(
  ...
  (error) => {
    if (import.meta.env.DEV) console.error('Response Error:', error.response?.status, error.response?.data || error.message)
    return Promise.reject(error)
  }
)
...
export const apiService = {
  ...
  /**
   * Knowledge base QA (alias)
   * @deprecated 请直接使用 knowledgeQuery()
   */
  async knowledgeQA(query, sessionId = 'default', model = null, collection = null, forceMultiDoc = null, keywordFilter = null, queryImage = null) {
